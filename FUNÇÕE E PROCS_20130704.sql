...
(
	@CAMPO1 VARCHAR(250),
	@CAMPO2 VARCHAR(250),
	@CAMPO3 VARCHAR(250)=NULL,
	@CAMPO4 VARCHAR(250)=NULL
...
SET  
	@RETORNO = RTRIM(LTRIM( ISNULL(@CAMPO1,'')))+ ' '+RTRIM(LTRIM(ISNULL(@CAMPO2,'')))+' '+RTRIM(LTRIM(ISNULL(@CAMPO3,'')))+' '+RTRIM(LTRIM(ISNULL(@CAMPO4,''))) 
...
--SELECT [dbo].FN_CONCAT 'BISCOITO','CHEBEL','AMENDOIN'
...
CREATE FUNCTION [DBO].[FN_SOMENTE_TEXTO] ( @TEXTO VARCHAR(200) ) 
/****************************************************************
	RODRIGO EDUARDO - 20/05/2013
	FUNÇÃO PARA RETIRAR NUMEROS DE TEXTO. PERMANECE APENAS OS CARACTERES.

****************************************************************/  
RETURNS VARCHAR(200)  
AS
BEGIN
DECLARE @INICIAL INT,@FINAL INT,@VARLETRAS VARCHAR(80)
...
/********************************************************
	FUNÇÃO PARA PADRONIZAR E RETIRAR CARACTERES ESPECIAIS DO CNPJ
	RODRIGO - 18/03/2013
...
/********************************************************
	FUNÇÃO PARA PADRONIZAR E RETIRAR ACENTUAÇÃO DE NOMES
	-CIDADE, BAIRRO, NOMES PRÓPRIOS
	RODRIGO - 18/03/2013
 ...
 DECLARE @TEXTO_FORMATADO VARCHAR(250)
	...
	ELSE
		BEGIN	 
			...
			-- O trecho abaixo possibilita que caracteres como "º" ou "ª"   -- sejam convertidos para "o" ou "a", respectivamente
...
CREATE PROCEDURE dbo.DROPOBJETO
	@OBJETO varchar(100), 
	@TIPO varchar(2) = 'U', 
	@ESPACO VARCHAR(100) = ''
AS
BEGIN
	DECLARE @TASK VARCHAR(300),
			@QUERY VARCHAR(300)
	SET @TASK = ''
	DECLARE @ERRO INT
	DECLARE @TEMPORARIA VARCHAR(100)
	SET @TEMPORARIA = ''

	IF @TIPO <> 'U' AND @TIPO <> 'P' AND @TIPO <> 'V' AND @TIPO <> 'FN' AND @TIPO <> 'TR' AND @TIPO <> 'IF' AND @TIPO <> 'TF'
		BEGIN
			PRINT 'ERROR: OBJETO NÃO SUPORTADO'
		END
	ELSE
		BEGIN
				...
				BEGIN
					...
					IF OBJECT_ID(@TEMPORARIA) IS NOT NULL
						BEGIN
							...
							EXEC(@TASK)
							SET @QUERY = @ESPACO + @OBJETO + ' excluído com sucesso'
							PRINT @QUERY
						END
				END
			ELSE
				BEGIN
						...
						BEGIN
							...
							IF (@ERRO <> 0)
								BEGIN
									IF (@ERRO <> 547)
										BEGIN
											SET @QUERY = @ESPACO + 'Violação de CONSTRAINT'
											PRINT @QUERY
										END
									ELSE
										BEGIN
					...
					ELSE
						BEGIN
							SET @QUERY = @ESPACO + 'ERROR: Objeto Não Existe'
							PRINT(@QUERY)
						END
				END
		END
...
CREATE PROCEDURE [dbo].[SP_PROCURA_OBJETO]
	@pOBJETO SYSNAME
AS
BEGIN

	SELECT @pOBJETO, NAME FROM SYS.OBJECTS WHERE NAME LIKE '%' + @pOBJETO + '%'	
...
Text
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[SP_VERIFICA_INDICE]
	@pOBJETO SYSNAME
AS
BEGIN
	...
	SELECT	NAME AS INDEXNAME ,
			...
			CASE IS_DISABLED WHEN 0 THEN 'ENABLED'
				 ELSE 'DISABLED'
			END INDEXUSAGE,
			FILL_FACTOR
	FROM	SYS.INDEXES
	WHERE	OBJECT_ID = OBJECT_ID(@pOBJETO)
	ORDER BY 1,3
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------
-- VERIFICA FRAGMENTAÇÃO DO INDICE
----------------------------------------
CREATE PROCEDURE [dbo].[SP_VERIFICA_INDICE_FRAG]
	@pOBJECTNAME SYSNAME
AS
BEGIN
	...
	SELECT	A.INDEX_ID,
			B.NAME,
			INDEX_COL(@pOBJECTNAME,A.INDEX_ID,1) INDEX_COLUMN,
			A.AVG_FRAGMENTATION_IN_PERCENT
	FROM	SYS.DM_DB_INDEX_PHYSICAL_STATS (DB_ID(),OBJECT_ID(@pOBJECTNAME),NULL,NULL,NULL) AS A
...
BEGIN
	DECLARE @IDADE INT
	SET @IDADE = CASE
						WHEN MONTH(@dataNascimento) < MONTH(@dataAtual) THEN DATEDIFF(YY, @dataNascimento,@dataAtual) 
						WHEN MONTH(@dataNascimento) > MONTH(@dataAtual) THEN DATEDIFF(YY, @dataNascimento,@dataAtual) - 1
						WHEN MONTH(@dataNascimento) = MONTH(@dataAtual) AND DAY(@dataNascimento) <= DAY(@dataAtual) THEN DATEDIFF(YY, @dataNascimento,@dataAtual) 
						WHEN MONTH(@dataNascimento) = MONTH(@dataAtual) AND DAY(@dataNascimento) > DAY(@dataAtual) THEN DATEDIFF(YY, @dataNascimento,@dataAtual) - 1
				END
	RETURN @IDADE
END
